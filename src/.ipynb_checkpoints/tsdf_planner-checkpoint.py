...
from src.epistemic_graph import (
    ...
    NodeType,
    NodeStatus,
    create_ghost_node_from_frontier,
)
from src.ghost_node_predictor import GhostNodePredictor, compute_semantic_potential_score
...
class Frontier:
    ...
    # Phase I: Epistemic Graph Refinement - Ghost Node semantic distribution
    semantic_dist: object = None  # SemanticDistribution object from epistemic_graph.py
    ghost_node_id: str = None  # Corresponding ghost node ID in epistemic graph
...
class TSDFPlanner(TSDFPlannerBase):
    ...
    def __init__(
        ...
        epistemic_graph_cfg=None,  # Phase I: Configuration for Epistemic Graph
    ):
        super().__init__(
            vol_bnds,
            voxel_size,
        ...
        # Phase I: Epistemic Graph Refinement - Initialize modules
        if epistemic_graph_cfg is None:
            epistemic_graph_cfg = {
                "semantic_residual_threshold": 0.5,
                "min_dependency_confidence": 0.6,
                "enable_cascade_deletion": True,
                "enable_vlm_ghost_prediction": True,
                "ghost_node_top_k_categories": 5,
                "use_spatial_context": True,
                "use_visual_cues": True,
            }
        self.epistemic_graph_cfg = epistemic_graph_cfg
        self.epistemic_graph = EpistemicGraph(cfg=epistemic_graph_cfg)
        self.ghost_node_predictor = GhostNodePredictor(cfg=epistemic_graph_cfg)
        self.enable_epistemic_refinement = epistemic_graph_cfg.get(
            "enable_epistemic_refinement", True
        )

        # Track observation history for Ghost Node prediction
    ...
    def update_frontier_map(
        ...
        # Phase I: Epistemic Graph Refinement - Predict semantic distribution for Ghost Nodes
        if self.enable_epistemic_refinement:
            self._predict_ghost_node_semantics(scene, pts_habitat)
    ...
    # ============================================================================
    # Phase I: Epistemic Graph Refinement - Ghost Node Semantic Prediction
    # ============================================================================

    def _predict_ghost_node_semantics(self, scene, pts_habitat):
        """
        Predict semantic distributions for all frontiers (Ghost Nodes)
        ...
        predictions = self.ghost_node_predictor.batch_predict_frontiers(
            frontiers=self.frontiers,
            spatial_context=spatial_context,
            observed_history=self.observation_history
        )

        # Assign semantic distributions to frontiers and create Ghost Nodes
        for frontier in self.frontiers:
            if frontier.frontier_id in predictions:
                semantic_dist = predictions[frontier.frontier_id]
                frontier.semantic_dist = semantic_dist

                # Create Ghost Node and add to Epistemic Graph
                ghost_node = create_ghost_node_from_frontier(
                    frontier=frontier,
                    semantic_dist=semantic_dist,
                    parent_node_id=None  # No parent for initial frontiers
                )
                node_id = self.epistemic_graph.add_node(ghost_node)
                frontier.ghost_node_id = node_id

                print(f"[Phase I] Created Ghost Node {node_id} for Frontier {frontier.frontier_id}")
    ...
    def _build_spatial_context(self, scene, pts_habitat) -> Dict:
        """
        Build spatial context for Ghost Node prediction
        """
        spatial_context = {
            "nearby_objects": [],
    ...
    def update_observation_history(self, snapshot: SnapShot, room_type: Optional[str] = None):
        """
        Update observation history for Ghost Node prediction
        """
        observation = {
    ...
    def get_epistemic_graph_statistics(self) -> Dict:
        """
        Get statistics from the Epistemic Graph
        """
        return self.epistemic_graph.get_statistics()
